...
class DownExcel{
    ...
    public function setExcelHeader($headerArr){
        if(!empty($headerArr)){
            $endStr = $this->getColumn(count($headerArr)-1);
            $this->objPHPExcel->getActiveSheet()->getStyle("A{$this->current_row}:{$endStr}".($this->current_row))->applyFromArray(
                array(
    ...
    public function setExcelData($data,$keyList){
        if(!empty($data)){
            $startRow = $this->current_row;
            $endStr = $this->getColumn($this->th_num-1);
            foreach ($data as $list){
                if(key_exists("row_height",$list)){
                    $this->objPHPExcel->getActiveSheet()->getRowDimension($this->current_row)->setRowHeight($list["row_height"]);
                }
                $col = 0;
                foreach ($keyList as $key=>$item){
                    $text = key_exists("{$key}",$list)?$list[$key]:"";
                    $this->objPHPExcel->getActiveSheet()
            ...
            $this->current_row--;
            $this->objPHPExcel->getActiveSheet()->getStyle("A{$startRow}:{$endStr}".($this->current_row))->applyFromArray(
                array(
                    'alignment'=>array(
    ...
    //冻结某行
    public function setFreezePane($str){
